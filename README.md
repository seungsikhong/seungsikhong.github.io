...
명령을 실행하면 제목, 요약, 카테고리, 태그를 순서대로 물어봅니다. 카테고리와 태그는 번호로 선택할 수 있고, 태그는 쉼표로 여러 개를 입력할 수 있습니다.
...
```bash
npm run blog:new -- --title "글 제목" --excerpt "글 요약" --category "카테고리" --slug my-post
```
...
`blog:new`은 등록되지 않은 카테고리나 태그를 받으면 새 값으로 자동 추가합니다. 비슷한 기존 값이 있으면 오타 방지를 위해 기존 값을 사용할지 새로 추가할지 물어봅니다.
